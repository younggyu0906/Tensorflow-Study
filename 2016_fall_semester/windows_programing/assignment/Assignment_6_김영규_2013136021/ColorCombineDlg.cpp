...
BOOL CColorCombineDlg::OnInitDialog()
{
	...
	CKYGImageToolDoc* pDoc = NULL;
	POSITION pos = pApp->m_pImageDocTemplate->GetFirstDocPosition();

	while (pos != NULL)
	{
		pDoc = (CKYGImageToolDoc*)pApp->m_pImageDocTemplate->GetNextDoc(pos);
...
//		SendMessage(WM_COMMAND, IDCANCLE, 0);
	}
...
void CColorCombineDlg::OnBnClickedOK()
{
	...
	m_pDoc3 = (CKYGImageToolDoc*)m_comboImage3.GetItemDataPtr(m_comboImage3.GetCurSel());
