...
IMPLEMENT_DYNCREATE(CkeyView, CView)
	...
	CkeyView::CkeyView()
	{
		...
		yMax = 0;
		choice = 0;
		m_rectangleCursor = false;
		m_ellipseCursor = false;
		m_lineCursor = false;
	}
	...
	void CkeyView::OnMouseMove(UINT nFlags, CPoint point)
	{
		...
		if(DrawMove == true) {
			...
			if(choice == 1) {
				m_rectangleCursor = TRUE;
				dc.Rectangle(SP.x, SP.y, EP.x, EP.y);
			}
			else if(choice == 2) {
				m_ellipseCursor = TRUE;
				dc.Ellipse(SP.x, SP.y, EP.x, EP.y);
			}
			else {
				m_lineCursor = TRUE;
	...
	BOOL CkeyView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
	{
		if(choice == 1)
			SetCursor(LoadCursor(NULL, m_rectangleCursor ? IDC_RECTANGLE : IDC_ARROW));
		else if(choice == 2)
			SetCursor(LoadCursor(NULL, m_ellipseCursor ? IDC_ELLIPSE : IDC_ARROW));
		else
			SetCursor(LoadCursor(NULL, m_lineCursor ? IDC_LINE : IDC_ARROW));
		return TRUE;
