...
: m_pNewDib(NULL), m_pImageDocTemplate(NULL)	// 7단원 추가내용 (m_pImageDocTemplate(NULL))멤버 초기화리스트 (포인터 초기화)
{
}
...
BOOL CImageToolApp::InitInstance()
{
	...
	m_pImageDocTemplate = new CMultiDocTemplate(IDR_ImageToolTYPE,	// 7단원 추가 내용
		RUNTIME_CLASS(CImageToolDoc),
		RUNTIME_CLASS(CChildFrame), // 사용자 지정 MDI 자식 프레임입니다.
		RUNTIME_CLASS(CImageToolView));
	...
	AddDocTemplate(m_pImageDocTemplate);	// 7단원 추가 내용
